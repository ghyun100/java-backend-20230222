...
// 컴파일 및 로직상의 문제는 없지만 사용자의 입력 또는 타 프로그램의 결과에서
...
// 예외 : 정상적인 입력에서는 정상 처리가 되지만 비정상적인 입력에서는 처리가 되지 않는 문제 (null 값 참조, 로직 실행 중 데이터 베이스 연결 실패 등)
...
//		System.out.println(numbers[4]);
		...
		} catch (NullPointerException exception) { //(NullPointerException 말고 Exception, RuntimeException가능)
		...
		try {
			...
			exception.printStackTrace(); // 에러의 발생근원지를 찾아서 단계별로 에러를 출력
		...
		try {
			bringException();// 이거만 치면 빨간줄, 빨간줄 두번째 클릭
		} catch (Exception e) {
